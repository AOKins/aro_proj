...
//	TRACE("CDib::MakePalette -- m_nColorTableEntries = %d\n", m_nColorTableEntries);
	...
	if (pLogPal == NULL)
	{
		AfxMessageBox(L"Error allocating memory for MakePalette", MB_OK);
	}
...
BOOL CDib::Read(CFile* pFile)
{
	...
	try {
		...
		if(nCount != sizeof(BITMAPFILEHEADER)) {
			...
			AfxMessageBox(L"Size of BITMAPFILEHEADER Incorrect, Read", MB_OK);
		}
		if(bmfh.bfType != 0x4d42) {
			//throw new CException;
			AfxMessageBox(L"Incorrect Bitmap Type,Read", MB_OK);
		}
		nSize = bmfh.bfOffBits - sizeof(BITMAPFILEHEADER);
		m_lpBMIH = (LPBITMAPINFOHEADER) new char[nSize];
		if (m_lpBMIH == NULL)
		{
			AfxMessageBox(L"Error allocating memory for CDib,Read", MB_OK);
		}
		...
		if (m_lpImage == NULL)
		{
			AfxMessageBox(L"Error allocating memory for image CDib:Read", MB_OK);
		}
...
void CDib::ComputeMetrics()
{
	if(m_lpBMIH->biSize != sizeof(BITMAPINFOHEADER)) {
		...
		AfxMessageBox(L"Size of BITMAPFILEHEADER Incorrect, ComputeMetrics", MB_OK);
		//throw new CException;
	}
